describe Fluent::ParsePostfixFilter do
  ...
  context 'when cannot parse' do
    ...
    it do
      is_expected.to match_array [
        ["test.default", 1432492200, {"message"=>"Feb 27 09:02:37 MyHOSTNAME postfix/smtp[26490] x D53A72713E5: to=<myemail@bellsouth.net>, relay=gateway-f1.isp.att.net[204.127.217.16]:25, delay=0.57, delays=0.11/0.03/0.23/0.19, dsn=2.0.0, status=sent (250 ok ; id=20120227140036M0700qer4ne)"}],
  ...
  context 'include hash (sha1)' do
    let(:fluentd_conf) do
      {include_hash: true, salt: 'my_salt', sha_algorithm: 1}
    end
  ...
  context 'when error happen' do
    ...
    it do
      is_expected.to match_array [
        ["test.default", 1432492200, {"message"=>"Feb 27 09:02:37 MyHOSTNAME postfix/smtp[26490]: D53A72713E5: to=<myemail@bellsouth.net>, relay=gateway-f1.isp.att.net[204.127.217.16]:25, conn_use=2, delay=0.57, delays=0.11/0.03/0.23/0.19, dsn=2.0.0, status=sent (250 ok ; id=20120227140036M0700qer4ne)"}],
